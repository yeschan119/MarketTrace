...
같은 사건 모델로 다루는 시스템. **1~3단계 완료, 4단계(신호·검증) ~88%, 5단계(사용자) ~64%.**
코퍼스 461건 완주로 검정력 확보 → **방향 A(측정·검증 우선) 검증 통과**: 유의 사건유형 9버킷,
워크포워드 백테스트 5일 순수익 +2.1%(표본외). calibration·검증신호 개별 노출·사람 검토 루프까지
완료 → 초점은 이제 **종목 단위 매수판단 집계(Phase 5)**.
...
| **4. 신호와 검증** | 🟡 ~88% | 유의성 t검정 + 워크포워드 백테스트(거래비용) + **5개 신호모델 비교** + 거시 분해 + **calibration** + **검증신호 개별 이벤트 노출(충돌 판정)**. 검증 통과. 남음(데이터 선행): 잔여 모델(펀더/가격/수급)·상폐 모델링 |
| **5. 사용자 기능** | 🟡 ~64% | 타임라인·상세·`/stats`(Validated Signals·5모델 백테스트·거시 분해·보정) + **사건 단위 검증신호 결합·미검토 충돌 검토 큐**. 남음: **종목별 매수판단 화면**·알림·시나리오 |
...
## 4. 다음 단계 — 종목 단위 매수판단 집계

> calibration·검증신호 개별 노출·사람 검토 루프까지 완료. 신뢰 신호는 사건 기반.
> 검증신호가 사건·통계 단위까지 노출됨 → 남은 핵심은 **종목(instrument) 단위 집계**.

1. **종목별 매수판단 화면 (추천, Phase 5)** — 종목 타임라인 뷰에 그 종목의 사건들을 검증신호로 집계한 **종합 판정**(예: 최근 유의 사건 방향·충돌 수·기대 드리프트)을 결합. 현재는 사건·통계 단위까지만.
2. **잔여 SignalModel (데이터 선행)** — 펀더멘털·가격·수급을 동일 `SignalModel`로. **수급/상폐 데이터 수집이 선행돼야 함**(상폐는 delisted_at 미채움·provider 미수집으로 현재 모델링 불가).
3. **골드셋 확대 → 라이브 F1** — 분류/기업연결 실측 정확도 측정(현재 프레임워크만).
4. **알림·시나리오** — 관심종목 알림, 시나리오별 상승·하락 요인.
...
## 6. 이번 세션에 한 일 (2026-07-04 — 검증신호 노출 → 사람 검토 루프)
