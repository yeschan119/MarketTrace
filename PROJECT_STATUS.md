...
| **5. 사용자 기능** | 🟡 ~76% | 타임라인·상세·`/stats` + 사건 단위 검증신호 결합·검토 큐 + **종목별 매수판단 화면(종합 판정 + 상승·하락 요인 분해)**. 남음: 관심종목 알림·가중 정교화 |
...
> 검증신호 전 층위 노출(사건·목록·통계·종목) + 사람 검토 루프 + 종목 매수판단(판정+요인)까지 완료.
...
2. **골드셋 확대 → 라이브 F1** — 분류/기업연결 실측 정확도 측정(골드셋 6건 → 수동 라벨 확대 필요).
3. **관심종목 알림** — 알림 인프라(Telegram 등) 필요.
4. **종목 매수판단 정교화** — 확신도·최근성 가중, 종목 간 랭킹.
