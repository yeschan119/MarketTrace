...
| **4. 신호와 검증** | 🟡 ~72% | 유의성 t검정 + 워크포워드 백테스트(거래비용) + **5개 신호모델 비교** + 거시 분해. 검증 통과. 남음: calibration·잔여 모델(펀더/가격/수급)·상폐 모델링 |
| **5. 사용자 기능** | 🟡 ~52% | 타임라인·상세·`/stats`(Validated Signals 패널·5모델 백테스트·거시 분해 표). 남음: 종목별 매수판단 화면·알림·시나리오 |
...
## 4. 다음 단계 — 4단계 마무리 + 매수판단 노출

> 검증(방향 A) 통과 + 5모델 비교 완료(거시=프록시 확증). 신뢰 신호는 사건 기반.
> 검증된 신호 UI 노출 + significant/event_type/macro/combined/llm 모델·거시 분해는 **완료**.

1. **calibration** (추천) — confidence 구간별 실제 적중률(0.7 ≈ 실제 70%). 신호 품질이 제각각이라 결합·신뢰 전 정량화 필수
2. **매수판단 노출(Phase 5)** — 검증된 사건유형(significant)을 종목별 매수판단 화면으로(현재는 /stats 통계 페이지에만)
3. **잔여 모델·상폐 모델링** — 펀더멘털·가격·수급을 동일 `SignalModel`로 + 거래정지·상장폐지를 수익률에 반영
